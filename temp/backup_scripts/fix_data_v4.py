...
excel_mapping_path = os.path.join(base_dir, 'temp', 'excel_mapping_v4.json')
thumb_index_path = os.path.join(base_dir, 'temp', 'thumb_index.json')
backup_dir = os.path.join(base_dir, 'temp', 'backup')
os.makedirs(backup_dir, exist_ok=True)

# 备份原始文件
backup_path = os.path.join(backup_dir, 'index.html.backup.v4')
    ...
    if matches:
        ...
        for m in matches:
            if not m['variant']:
                best = m
                break
        excel_to_file_list.append((excel_map, best['path']))
    else:
        missing_in_thumb.append(excel_map)

print(f'Excel mappings with file: {len(excel_to_file_list)}')
...
for item in data:
    ...
    # 在 excel_to_file_list 中查找匹配
    found = False
    for excel_map, file_path in excel_to_file_list:
...
# 保存修正日志
log_path = os.path.join(base_dir, 'temp', 'fix_log_v4.json')
